...
shmwriterProcessor::shmwriterProcessor() :_filepath("/dev/shm/shmwriter"),_started(false)
{
...
void shmwriterProcessor::start(uint32_t run)//,std::string dir,std::string setup)
{

  _run=run;
  _started=true;
}
void shmwriterProcessor::processRunHeader(std::vector<uint32_t> header)
{
 if (!_started) return;
...
void shmwriterProcessor::loadParameters(Json::Value params)
{
      _filepath=params["eudaqshmpath"].asString();
}
void shmwriterProcessor::processEvent(uint32_t gtc,std::vector<zdaq::buffer*> vbuf)//writeEvent(uint32_t gtc,std::vector<unsigned char*> vbuf)
{
  if (!_started) return;
...
void shmwriterProcessor::stop()
{

}

void shmwriterProcessor::ls(std::string sourcedir,std::vector<std::string>& res)
{
...
void shmwriterProcessor::store(uint32_t detid,uint32_t sourceid,uint32_t eventid,uint64_t bxid,void* ptr,uint32_t size,std::string destdir)
{
