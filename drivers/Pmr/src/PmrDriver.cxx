...
lydaq::PmrDriver::PmrDriver(char * deviceIdentifier, uint32_t productid ) 
{
  ...
  if ((ret = ftdi_usb_close(&theFtdi)) < 0)
    {
      ...
      LOG4CXX_ERROR(_logDIF,"unable to close ftdi device: "<<_deviceId);
      ftdi_deinit(&theFtdi);
...
int32_t lydaq::PmrDriver::open(char * deviceIdentifier, uint32_t productid ) 
{
  int32_t ret;
  ...
  if (ftdi_init(&theFtdi) < 0)
    {
      LOG4CXX_FATAL(_logDIF,"FTDI_INIT failed");
      return -1;
    }

  if ((ret = ftdi_usb_open_desc(&theFtdi, 0x0403,productid,NULL,deviceIdentifier)) < 0)
    {
      LOG4CXX_FATAL(_logDIF,"Cannot open ftdi device "<<deviceIdentifier<<" id "<<std::hex<<productid<<std::dec<<" rc:"<<ret);
      return -2;   
    }
...
int32_t lydaq::PmrDriver::writeNBytes( unsigned char  *cdata, uint32_t nb)
{
  int32_t tbytestowrite=nb;
  int32_t ret=ftdi_write_data(&theFtdi,cdata,tbytestowrite);
  if( ret==-666)
    {
      LOG4CXX_ERROR(_logDIF,"USB device unavailable");
    }
  if( ret<0)
    {
      LOG4CXX_ERROR(_logDIF,"usb_bulk_write error = "<<ret);
    }

  if (ret != nb) 
    {
      LOG4CXX_ERROR(_logDIF,"Timeout occured while writing on FT245");
    }
  return ret;
}

int32_t   lydaq::PmrDriver::readNBytes( unsigned char  *resultPtr,uint32_t nbbytes )
{
  ...
  if( ret<0)
    {
      LOG4CXX_ERROR(_logDIF,"usb_bulk_read error = "<<ret);
    }
  if (ret==0)
    {
      LOG4CXX_ERROR(_logDIF,"No data after ntry = "<<ntry);
  ...
  if (tbytesread != (int32_t) nbbytes) 
    {
      memset(resultPtr,0,nbbytes);
      LOG4CXX_ERROR(_logDIF,"Only  "<<ret<<" Bytes found, "<<nbbytes<<" required, buffer cleared");

    }
  return ret;
}

int32_t lydaq::PmrDriver::registerWrite(uint32_t address, uint32_t data)
{
  uint32_t taddress;
...
int32_t lydaq::PmrDriver::registerRead(uint32_t address, uint32_t *data)
{
  uint32_t taddress;
  ...
  if (writeNBytes(ttampon,2) !=2)		// write address 
    {
      (*data)=0;
      LOG4CXX_ERROR(_logDIF,"write adress error");	
      return -2;
    }
  ...
  if (readNBytes(ttampon,4)!=4)
    {
      (*data)=0;
      LOG4CXX_ERROR(_logDIF,"read error");
      return -1;
    }
...
int32_t lydaq::PmrDriver::setup()
{
  ...
  if (tdata!=0x12345678)
    LOG4CXX_ERROR(_logDIF,"Invalid Test register test "<<std::hex<<tdata<<std::dec);
...
int32_t lydaq::PmrDriver::loadSLC(unsigned char* SLC,uint32_t slc_size)
{
  int32_t ret;
  uint32_t taddr;
  uint32_t tdata;
...
int32_t lydaq::PmrDriver::setPowerPulsing(bool enable,uint32_t an2d,uint32_t d2ac,uint32_t ac2d,uint32_t d2an)
{
  int32_t ret;
...
int32_t lydaq::PmrDriver::resetFSM()
{
  int32_t ret;
...
int32_t lydaq::PmrDriver::readData(unsigned char* tro,uint32_t size)
{
