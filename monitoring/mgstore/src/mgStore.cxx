...
lydaq::mgStore::mgStore(): _dbaccount(""),_uri(0),_client(0),_database(0),_measitems(0) {} 
void lydaq::mgStore::loadParameters(Json::Value params)
{
  if (params.isMember("mgstore"))
    _params=params["mgstore"];
}

void lydaq::mgStore::connect()
{
   bson_t *command, reply;
  bson_error_t error;
  char *str;
  std::string login("");
  if (_params.isMember("login"))
    {
      login=_params["login"].asString();
    }
  else
    {
  ...
  */
  ...
  LOG4CXX_INFO(_logLdaq," Ping to "<<login<<" => "<<std::string(str));
  bson_destroy (&reply);
