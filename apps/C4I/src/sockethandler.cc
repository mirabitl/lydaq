...
uint32_t c4i::socketHandler::sendMessage(c4i::Message* m)
{
  ...
  try
  {
    ...
    m->ptr()[c4i::Message::Fmt::TRANS]=tr;
    // Clear the ack tag for reply
    _answ[tr][c4i::Message::Fmt::CMD]=0;
...
int16_t c4i::socketHandler::checkBuffer(uint8_t* b,uint32_t maxidx)
{
   uint32_t elen=0;
  if (b[0]!='(')  return -2;
 // Check TAG coherency
...
void c4i::socketHandler::processBuffer(uint64_t id, uint16_t l,char* bb)
{
...
void c4i::socketHandler::purgeBuffer()
{
