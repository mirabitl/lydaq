...
void febv1::dataHandler::autoRegister(zmq::context_t* c,Json::Value config,std::string appname,std::string portname)
{
  if (_dsData!=NULL)
    delete _dsData;
  _dsData = new zdaq::zmSender(c,_detId,this->sourceid());
...
bool febv1::dataHandler::processPacket()
{
  ...
  if (ntohl(_lBuf[0]) == 0xcafedade)
	...
	{
	  // Write Event
	  LOG4CXX_INFO(_logFeb, __PRETTY_FUNCTION__ << "Writing completed Event" << _event << " GTC " << _lastGTC << " ABCID " << _lastABCID << " Lines " << _chlines << " ID " << id());
	  // To be done
      ...
      if (abcid == _lastABCID)
	{
	  LOG4CXX_ERROR(_logFeb, __PRETTY_FUNCTION__ << id() << " ABCID HEADER ERROR " << abcid << "  last " << _lastABCID);
	}
      if (gtc == _lastGTC)
	{
	  LOG4CXX_ERROR(_logFeb, __PRETTY_FUNCTION__ << id() << " GTC HEADER ERROR " << gtc << "  last " << _lastGTC);
	}
      _nProcessed++;
      _event++;
      _lastGTC = gtc;
...
#undef DEBUGPACKET
  ...
  LOG4CXX_DEBUG(_logFeb, __PRETTY_FUNCTION__ << id() << " Packets=" << _nProcessed << " channel=" << channel << " GTC=" << gtc << " lines=" << nlines << " index=" << _idx);
  ...
  if (_dsData != NULL)
    {
      ...
      if (_event % 100 == 0)
	LOG4CXX_INFO(_logFeb, __PRETTY_FUNCTION__ << id() << "Publish  Event=" << _event << " GTC=" << _lastGTC << " ABCID=" << _lastABCID << " Lines=" << _chlines << " size=" << idx);
    }
}
