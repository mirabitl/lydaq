...
void mbmdcc::registerHandler::writeRegister(uint16_t address,uint32_t value)
{
  ...
  sp[0]=htons(len);
  _msg->ptr()[mbmdcc::Message::Fmt::CMD]=lydaq::mbmdcc::Message::command::WRITEREG;
...
uint32_t mbmdcc::registerHandler::readRegister(uint16_t address)
{
  ...
  _msg->ptr()[mbmdcc::Message::Fmt::HEADER]='(';
  sp[0]=htons(len);
  _msg->ptr()[mbmdcc::Message::Fmt::CMD]=lydaq::mbmdcc::Message::command::READREG;
...
void mbmdcc::registerHandler::processReply(uint32_t tr,uint32_t* reply)
{
  ...
  int cnt=0;
  while (rep[mbmdcc::Message::Fmt::CMD]!=lydaq::mbmdcc::Message::ACKNOWLEDGE )
    {
      usleep(1000);
      cnt++;
      if (cnt>1000)
